...
ARG ACT_RUNNER_VERSION=v1.0.8
RUN git -C /root clone -b "$ACT_RUNNER_VERSION" --single-branch --depth=1 https://gitea.com/gitea/runner.git
WORKDIR /root/runner
...
COPY --from=builder /root/runner/gitea-runner /usr/bin/
COPY override /
